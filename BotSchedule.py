...
async def create_event(ctx: commands.Context):
    ...
    try:
        ...
        try:
            ...
            await ctx.author.send("❌ I don't have permissions to send messages in that channel!")
            return
        except discord.HTTPException as e:
            await ctx.author.send(f"❌ Failed to create event: {str(e)}")
            return
        for role_data in creation.data['roles'].values():
            try:
                await message.add_reaction(role_data['emoji_str'])
            except discord.Forbidden:
                await ctx.author.send("❌ I need 'Add Reactions' permission to set up roles!")
...
async def edit_event_time(ctx, event_id: str):
    ...
    try:
        ...
        await channel.send(f"⏰ The event time has been updated to {new_time.strftime('%Y-%m-%d %H:%M %Z')}! Participants have been notified.")
        for participant_role, participants in event['participants'].items():
            for user_id in participants:
                user = await bot.fetch_user(user_id)
                await user.send(f"⏰ The event time has been updated to {new_time.strftime('%Y-%m-%d %H:%M %Z')}!")
...
async def on_raw_reaction_add(payload):
    ...
    # Check if this emoji is for a role
    if emoji_key not in event['roles']:
        # Remove reaction if it's not a valid role emoji
        try:
            guild = bot.get_guild(payload.guild_id)
            member = await guild.fetch_member(payload.user_id)
            channel = bot.get_channel(payload.channel_id)
    ...
    if await already_signed_up(event, user_id_str):
        await message.remove_reaction(payload.emoji, member)
        await member.send(f"⚠️ You can only sign up for **one role** per event!\nRemove your existing signup first to change roles.")
        return

    if user_id_str in participants:
        await message.remove_reaction(payload.emoji, member)
        await member.send(f"⚠️ You're already signed up as {role_info['name']}!")
        return
    if user_id_str in waitlist:
        await message.remove_reaction(payload.emoji, member)
        await member.send(f"⚠️ You're already in the waitlist for {role_info['name']}!")
        return

    # Check role requirements
    if not await check_role_requirements(member, role_info['name']):
        await message.remove_reaction(payload.emoji, member)
        await member.send(f"❌ You need the {role_info['required_role']} role to sign up as {role_info['name']}!")
        return

    # Add to participants or waitlist
    if len(participants) >= role_info['slots']:
        event['waitlist'].setdefault(emoji_key, []).append(user_id_str)
        await member.send(f"⏳ {role_info['name']} is full! You're position #{len(event['waitlist'][emoji_key])} in waitlist.")
    else:
        event['participants'].setdefault(emoji_key, []).append(user_id_str)
        await member.send(f"✅ Successfully signed up as {role_info['name']}!")
...
async def on_raw_reaction_remove(payload):
    """
    Handles when a user removes a reaction from an event message.
    Removes the user from the role or waitlist and promotes waitlisted users.
    """
    if payload.user_id == bot.user.id:
        return
    ...
    if user_id_str in participants:
        ...
        event['participants'][emoji_key].remove(user_id_str)
        
        # If there's someone in waitlist, promote them
        if waitlist:
            next_user = event['waitlist'][emoji_key].pop(0)
            event['participants'].setdefault(emoji_key, []).append(next_user)
            
            # Notify promoted user
            try:
                user = await bot.fetch_user(int(next_user))
                await user.send(f"🎉 You've been promoted from waitlist to **{role_info['name']}** for event: **{event['name']}**!")
            except:
                pass
        ...
        try:
            ...
            await user.send(f"✅ You've been removed from **{role_info['name']}** for event: **{event['name']}**.")
        except:
            pass
    
    # Check if user is in waitlist
    elif user_id_str in waitlist:
        # Remove from waitlist
        event['waitlist'][emoji_key].remove(user_id_str)
        
        # Notify removed user
        try:
            user = await bot.fetch_user(payload.user_id)
            await user.send(f"✅ You've been removed from the **{role_info['name']}** waitlist for event: **{event['name']}**.")
        except:
            pass
    ...
    for event_id, event in list(events.items()):
        ...
        if (event['time'] - timedelta(hours=1)) < now < event['time']:
            try:
                channel = bot.get_channel(event['channel_id'])
                mentions = []
                for role_key, participants in event['participants'].items():
                    # Check if role still exists (in case roles were modified)
                    if role_key in event['roles']:
                        mentions.extend([f"<@{p}>" for p in participants])
                
                if mentions:  # Only send if there are participants
                    await channel.send(
...
async def on_command_error(ctx, error):
    ...
    await ctx.send(f"❌ Error executing command: {str(error)}", delete_after=15)
